...
public class UserService {
    ...
    public boolean changePassword(String email, String oldRawPassword, String newRawPassword) {
        if (email == null || oldRawPassword == null || newRawPassword == null) return false;
        var uOpt = userRepository.findByEmail(email);
        if (uOpt.isEmpty()) return false;
        User u = uOpt.get();
        try {
            // verify old password
            if (!passwordEncoder.matches(oldRawPassword, u.getPassword())) {
                try { System.out.println("[UserService] changePassword: old password mismatch for " + email); } catch (Throwable t) {}
                return false;
            }
            // validate new password policy
            if (!isValidPassword(newRawPassword)) {
                try { System.out.println("[UserService] changePassword: new password does not meet policy for " + email); } catch (Throwable t) {}
                return false;
            }
            ...
            try {
                if (notificationUrl != null && !notificationUrl.isBlank()) {
                    ...
                    payload.put("variables", vars);
                    notificationClient.sendEmailRequest(payload)
                        .doOnError(e -> { try { System.err.println("[UserService] send PASSWORD_CHANGED error: " + e.getMessage()); } catch (Throwable t) {} })
                        .subscribe();
                }
            } catch (Exception ex) {
                try { System.err.println("[UserService] changePassword notification error: " + ex.getMessage()); } catch (Throwable t) {}
            }

            return true;
        } catch (Exception ex) {
            try { System.err.println("[UserService] changePassword error: " + ex.getMessage()); ex.printStackTrace(); } catch (Throwable t) {}
            return false;
        }
    }
