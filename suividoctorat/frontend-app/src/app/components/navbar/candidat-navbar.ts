...
@Component({
  selector: 'candidat-navbar',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './candidat-navbar.html',
  styles: [`
    /* ========== CSS Variables ========== */
    :host {
      --navbar-height: 64px;
...
export class CandidatNavbarComponent implements OnInit {
  ...
  navItems = [
    { path: '/candidat/dashboard', icon: 'home', label: 'Tableau de bord' },
    { path: '/candidat/documents', icon: 'documents', label: 'Documents' },
    { path: '/candidat/soutenances', icon: 'presentation', label: 'Soutenances' },
    { path: '/candidat/campaigns', icon: 'megaphone', label: 'Campagnes' },
    { path: '/candidat/profile', icon: 'user', label: 'Profil' }
  ];
  ...
  loadProfile(): void {
    try {
      this.auth.getProfile().subscribe({
        ...
        error: (err) => {
          console.error('[CandidatNavbar] Failed to load profile:', err);
      ...
      console.error('[CandidatNavbar] Error loading profile:', error);
      this.profileImage = null;
      this.userEmail = null;
      this.userName = null;
      this.userInitials = '?';
  ...
  toggleTheme(): void {
    try {
      ...
      console.error('[CandidatNavbar] Error toggling theme:', error);
    }
  }
  ...
  changeLanguage(): void {
    try {
      ...
      console.log('[CandidatNavbar] Language change requested');
      // Example: Show a dropdown menu or toggle between languages
      const currentLang = localStorage.getItem('language') || 'fr';
      const newLang = currentLang === 'fr' ? 'en' : 'fr';
      localStorage.setItem('language', newLang);
      // You might want to use a translation service here
    } catch (error) {
      console.error('[CandidatNavbar] Error changing language:', error);
    }
  }
  ...
  logout(): void {
    try {
      ...
      console.error('[CandidatNavbar] Logout failed:', error);
  ...
  onImageError(event: any): void {
    ...
    try {
      ...
      console.error('[CandidatNavbar] Error handling image error:', error);
    }
  }

  getNavIcon(icon: string): SafeHtml {
    const icons: Record<string, string> = {
      ...
      documents: '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" stroke="currentColor" stroke-width="2" fill="none"/><polyline points="13 2 13 9 20 9" stroke="currentColor" stroke-width="2"/>',
      ...
      megaphone: '<path d="M3 11l18-5v12L3 13v-2z" stroke="currentColor" stroke-width="2" fill="none"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6" stroke="currentColor" stroke-width="2" fill="none"/>',
    ...
    // icons are static and defined in code, mark as trusted for innerHTML binding
