...
@Component({
  ...
  styles: [
    ...
    /* Save button specific override: green background and white text */
    .card .btn-save, .btn.btn-save { display:inline-flex !important; align-items:center !important; justify-content:center !important; gap:0.5rem !important; padding:0.5rem 0.8rem !important; font-weight:600 !important; background: #28a745 !important; color: #fff !important; border:1px solid #28a745 !important; border-radius:8px !important; }
    .card .btn-save:hover, .btn.btn-save:hover { background: #218838 !important; transform: translateY(-1px); box-shadow: 0 8px 18px rgba(33,136,56,0.18) !important; }
    /* Visible edit button next to icon to toggle edit mode */
    .btn-edit { padding: 0.5rem 0.8rem; font-size:0.9rem; }

     /* Ensure primary buttons in this component display correctly
       even if global/component styles from other parts of the app
       leak in or use different encapsulation. Higher specificity
       + !important guarantees legibility here. */
     .card .btn-primary, .btn.btn-primary { background: var(--blue-600) !important; color: #fff !important; box-shadow: none !important; }
     .card .btn-primary:hover, .btn.btn-primary:hover { background: var(--blue-700) !important; }
     .card .btn-primary:disabled, .btn.btn-primary:disabled { opacity: 0.75 !important; color: #fff !important; cursor: not-allowed !important; box-shadow: none !important; }
...
export class ProfilePage {
  ...
  changePassword() {
    ...
    this.auth.changePassword(this.oldPassword, this.newPassword, this.confirmNewPassword).subscribe({
      next: (res:any) => {
        ...
        // After password change, force logout to invalidate client session
        // and require re-authentication with the new password.
        try {
          console.info('[Profile] password changed; logging out user');
          // small delay so the success toast can be seen briefly
          setTimeout(() => {
            try { this.auth.logout(); } catch(e) { console.warn('[Profile] logout failed', e); }
            try { this.router.navigate(['/auth']); } catch(e) { console.warn('[Profile] navigation to /auth failed', e); }
          }, 900);
        } catch(e) {
          console.warn('[Profile] error during post-password-change logout', e);
        }
      },
      error: (err:any) => {
  ...
  onAvatar(e: any){
    ...
    this.http.post('/gestion-auth-service/api/auth/me/avatar', fd).subscribe({ 
      next: (res:any) => { 
        if (res?.avatar) { 
  ...
  saveProfile(){
    ...
    this.http.put('/gestion-auth-service/api/auth/me', payload).subscribe({ 
      next: (res:any) => { 
