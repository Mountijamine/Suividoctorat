...
export class AddDocumentPage {
  ...
  submit(){
    ...
    this.http.post('/api/documents', fd, { reportProgress:true, observe: 'events' }).subscribe({
      ...
      error: (err) => { this.uploading = false; this.error = err?.error?.message || 'Upload failed'; console.error('Upload failed', err); }
    });
  }
}
